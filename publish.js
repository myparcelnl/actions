...
(async() => {
  ...
  for (const v of versionTags) {
    ...
    await spawnSync(`git push origin :refs/tags/v${v}`, { stdio: 'inherit' });

    console.info(`Pushing new tag v${v} to git.`);
    await spawnSync(`git tag --force v${v}`, { stdio: 'inherit' });
    await spawnSync(`git push origin v${v}`, { stdio: 'inherit' });
  }
})();
